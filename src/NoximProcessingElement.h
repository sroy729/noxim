...
SC_MODULE(NoximProcessingElement)
{
	...
	int packet_queue_length;            //the number of the waiting packets in the NI queue (Jimmy added on 2012.04.24)
	int drop_pkt;                       //the number of the dropping packets, which are in NI. (Jimmy added on 2012.04.24)
	int inject_pkt;                     //the number of the packets which should be injected (Jimmy added on 2012.04.26)
	int t_quota;                        //Use to compare the complete time with fixed traffic quota (Jimmy added on 2012.04.27)
	int reshot_pkt;                     //Record the total number of reshot packets (Jimmy added on 2017.05.30)
	...
	int  sel_int_node_belt (int source, int destination, bool &beltway);
	int  sel_int_node_Mbelt(int source, int destination, bool &beltway);
	...
	void CalcMessageDelay( vector <int > & );
	...
	private:
		...
		int     _Transient_transmit_tmp; //Jimmy added on 2017.05.30
		bool    _full_pkt; //Jimmy added on 2017.05.30
		NoximCoord flit_src; //Jimmy added on 2017.05.30
		...
		void    _flit_static(NoximFlit flit, int _receive_flit); //Jimmy modified this function on 2017.05.30
};
