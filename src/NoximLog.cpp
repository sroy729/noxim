...
void NoximLog::Throughput   (             ){
	...
	if(!transient_log_throughput.is_open())cout<<"Cannot open "<< throughput_filename.c_str() <<endl;
		...
		//transient_log_throughput<<"Cycle \tRecivedFlits \tThroughput \tTotal Pkts Gen. \tTotal Drop Pkts \tThrot. Num. \tPartial Throt.(1/2) \tPartial Throt.(1/3) \tPartial Throt.(1/4) \tPartial Throt.(1/5) \tPartial Throt.(1/6) \tMax. Temp.\tAdaptive \tDOR \tDW \tMid_Adaptive \tMid_DOR \tMid_DW \tBeltway"<<endl;
		transient_log_throughput<<"Cycle \tRecivedFlits \tThroughput \tTotal Pkts Gen. \tTotal Reshot Pkts \tTotal Drop Pkts  \tThrot. Num. \tPartial Throt.(1/2) \tPartial Throt.(1/3) \tPartial Throt.(1/4) \tPartial Throt.(1/5) \tPartial Throt.(1/6) \tMax. Temp. \tbuffer_allocation_times"<<endl;
	}
}
...
#define DIRECTION_SEMI_LOCAL         7 //used for redirective pkt 
		*/
void NoximLog::WeightsMatrix( NoximNoC *n ){
	ofstream weights;
	if(!mkdir("results/Weights",0777))
		cout<<"Making new directory results/Weights"<<endl;
	string weights_filename = string("results/Weights");
	weights_filename = MarkFileName( weights_filename);
	weights.open(weights_filename.c_str(), ios::out);
	if(!weights.is_open())
		cout<<"Cannot open "<<weights_filename<<endl;
	else{

		// for(int z=0; z < NoximGlobalParams::mesh_dim_z; z++){
		// 	weights<<"Layer"<<z<<endl;
		// 	for(int y = 0; y < NoximGlobalParams::mesh_dim_y; y++){
		// 		for(int x = 0; x < NoximGlobalParams::mesh_dim_x; x++){
		// 			weights<<"ID: "<<n->t[x][y][z]->r->local_id<<endl;
		// 			weights<<"cnt_tamperd_value"<<endl;
		// 			// weights<<"cnt_recieved: "<<endl;
		// 			//print for each direction
		// 			weights<<"NORTH:"<<"\t"<<n->t[x][y][z]->r->cnt_tampered[0]<<"\t"<<n->t[x][y][z]->r->getRoutedFlits(0)<<endl;
		// 			weights<<"EAST:"<<"\t"<<n->t[x][y][z]->r->cnt_tampered[1]<<"\t"<<n->t[x][y][z]->r->getRoutedFlits(1)<<endl;
		// 			weights<<"SOUTH:"<<"\t"<<n->t[x][y][z]->r->cnt_tampered[2]<<"\t"<<n->t[x][y][z]->r->getRoutedFlits(2)<<endl;
		// 			weights<<"WEST:"<<"\t"<<n->t[x][y][z]->r->cnt_tampered[3]<<"\t"<<n->t[x][y][z]->r->getRoutedFlits(3)<<endl;
		// 			// weights<<"UP:"<<"\t"<<n->t[x][y][z]->r->cnt_tampered[4]<<"\t"<<n->t[x][y][z]->r->routed_flits[4]<<endl;
		// 			// weights<<"DOWN:"<<"\t"<<n->t[x][y][z]->r->cnt_tampered[5]<<"\t"<<n->t[x][y][z]->r->routed_flits[5]<<endl;
		// 			weights<<"\n";
		...
		// 	for(int y = 0; y < NoximGlobalParams::mesh_dim_y; y++){
		// 		for(int x = 0; x < NoximGlobalParams::mesh_dim_x; x++){
		// 			weights<<n->t[x][y][z]->r->local_id<<":"<<endl;
		// 			// weights<<"cnt_tamperd_value"<<endl;
		// 			// weights<<"cnt_recieved: "<<endl;
		// 			//print for each direction
		// 			for(int i = 0;i<4;i++){
		// 				weights<<"  -  "<<n->t[x][y][z]->r->cnt_tampered[i]<<endl;
		// 			}
