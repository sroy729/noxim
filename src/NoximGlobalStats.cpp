...
double NoximGlobalStats::getNwTotalAverageDelay(){
	...
	for (int x=0; x<NoximGlobalParams::mesh_dim_x; x++){
		noc->t[x][y][z]->r->CalcDelay( router_buffer_delay );
...
double NoximGlobalStats::getNiTotalAverageDelay(){
	...
	for (int y=0; y<NoximGlobalParams::mesh_dim_y; y++)
	for (int x=0; x<NoximGlobalParams::mesh_dim_x; x++){
		noc->t[x][y][z]->pe->CalcDelay( ni_packet_delay );
...
double NoximGlobalStats::getMsgTotalAverageDelay(){
	...
	for (int y=0; y<NoximGlobalParams::mesh_dim_y; y++)
	for (int x=0; x<NoximGlobalParams::mesh_dim_x; x++){
		noc->t[x][y][z]->pe->CalcMessageDelay( ni_msg_delay );
...
//+CHDR delay histogram -----------------------------------------------------
	...
	for (unsigned int k = 0 ; k < router_buffer_delay.size() ; k++ ){
		if(router_buffer_delay.size()){
			if (router_buffer_delay[k] > maxd)
				maxd = router_buffer_delay[k];
		}
	}
	for (unsigned int k = 0 ; k < ni_packet_delay.size() ; k++ ){
		if(ni_packet_delay.size()){
			if (ni_packet_delay[k] > maxd)
				maxd = ni_packet_delay[k];
		}
	}
	for (unsigned int k = 0 ; k < ni_msg_delay.size() ; k++ ){
		if(ni_msg_delay.size()){
			if (ni_msg_delay[k] > maxd)
				maxd = ni_msg_delay[k];
		}
	}
	
	//double interval = maxd/10000;
    double interval = 360;
	int hist_data = 1000;
	...
	double *DelayHist = new double[ hist_data + 1 ];
	double *DistHist  = new double[ max_dist ];
	//initialize
	for (int i = 0 ; i <= hist_data ; i++ )
		DelayHist[i]=0;
	...
	for (unsigned int k = 0 ; k < router_buffer_delay.size() ; k++ ){
		if(router_buffer_delay.size()){
			//get delay histogram
			unsigned int samples = (unsigned int)(router_buffer_delay[k]/interval);
			samples = ( samples > hist_data )? hist_data : samples;
			DelayHist[samples]++;
		}
    }
	for (unsigned int k = 0 ; k < ni_packet_delay.size() ; k++ ){
		if(ni_packet_delay.size()){
			//get delay histogram
			unsigned int samples = (unsigned int)(ni_packet_delay[k]/interval);
			samples = ( samples > hist_data )? hist_data : samples;
			DelayHist[samples]++;
		}
    }
	for (unsigned int k = 0 ; k < ni_msg_delay.size() ; k++ ){
		if(ni_msg_delay.size()){
			//get delay histogram
			unsigned int samples = (unsigned int)(ni_msg_delay[k]/interval);
			samples = ( samples > hist_data )? hist_data : samples;
			DelayHist[samples]++;
		}
    }
