...
//Sensor allocaion (Jimmy added on 2016.03.19)
#define SENSOR_ALLOCATE_NORMAL      0 //Every NoC node has its own thermal sensor
#define SENSOR_ALLOCATE_EVEN		1 //thermal sensor is allocated at the node, if node id is even
#define SENSOR_ALLOCATE_ODD			2 //thermal sensor is allocated at the node, if node id is odd
#define SENSOR_ALLOCATE_TABLE       3 //thermal sensor is allocated at the node, where is defined by user
#define INVALID_SENSOR_ALLOCATE	   -1 //invalide sensor allocation algo.

//Temperature reconstruction policy (Jimmy added on 2016.03.20)
#define TEMP_RECTOR_LINEAR          0 //Recover the full system temperature via linear combination
#define TEMP_RECTOR_REGRESSION      1 //Recover the temp. via linear regression
#define INVALID_TEMP_RECTOR        -1 //invalide temperature reconstruction policy


//Sensor allocaion (Jimmy added on 2015.05.27)
#define SENSOR_ALLOCATE_NORMAL      0 //Every NoC node has its own thermal sensor
...
#define ROUTING_DLADR                15
#define ROUTING_DLAR                 16
#define ROUTING_DLDR                 17
#define ROUTING_DOWNWARD_CROSS_LAYER 18
...
#define SEL_RCA                      3
#define SEL_HS                       4
#define SEL_PROPOSED                 5
#define SEL_THERMAL                  6 //Jimmy added on 2014.12.17
#define INVALID_SELECTION            -1

// Selection information (Jimmy added on 2012.06.26)
#define BUF                          0
#define BUF_SERVER                   1
...
#define INSEL_CS                     1
//TBDB Select
#define NO_BUFFER_ALLOCATION         0
#define BUFFER_ALLOCATION            1
#define INVALID_BUFFER_ALLOCATION    -1
//Throt_Reset_Buffer
#define NO_Throt_Reset_Buffer         0
#define DO_Throt_Reset_Buffer         1
#define INVALID_Throt_Reset_Buffer    -1
//TBDB_Type
#define TBDB_Type_Original            0
#define TBDB_Type_PRE                 1
#define TBDB_Type_GAME                2
#define TBDB_Type_GPRE                3
#define INVALID_TBDB_Type            -1
//TD_Type
#define TD_Type_Original             0
#define TD_Type_GTDAR                1
#define INVALID_TD_Type             -1
// Verbosity levels                  
...
#define TQUOTA_OFF                   0 //Use to compare the complete time with fixed traffic quota (Jimmy added on 2012.04.27)
#define TQUOTA_ON                    1 //Jimmy added on 2012.04.27
/****************MODIFY BY HUI-SHUN********************/
...
#define DEFAULT_STATIC_TQUOTA                    TQUOTA_OFF                       
...
#define DEFAULT_PACKET_QUEUE_DEPTH               8 //the depth of the queue in the NI
...
#define DEFAULT_SELECTION_INFO                   BUF //determine what kind of info. will be used for the selection: BUFFER or BUF_SERVER (Jimmy added on 2012.06.26)
...
#define DEFAULT_BUFFER ALLOCATION                NO BUFFER ALLOCATION
...
#define DEFAULT_THROTTLING_TYPE		        THROT_TAVT
...
#define DEFAULT_PREDICT_VOL                 2 //how far is the prediciton cap.(Jimmy addd on 2012.04.08)
#define DEFAULT_MESSAGE_LEVEL               true
#define DEFAULT_PACKET_QUEUE_LENGTH         4 
#define DEFAULT_TLA                         1 //0: w/o TLA scheme 1: w/ TLA scheme
#define TEMP_THRESHOLD                      98 //Jimmy added on 2012.11.06
#define DEFAULT_DFS_LEVEL                   2 //Jimmy added on 2013.03.06
#define DEFAULT_SENSOR_ALLOCATE             SENSOR_ALLOCATE_NORMAL //Jimmy added on 2015.05.27
#define DEFAULT_SENSOR_ALLOCATE_NUM         4 //Jimmy added on 2016.03.21
#define DEFAULT_SENSOR_TABLE_FILENAME       "sensor_placement.txt" //Jimmy added on 2016.03.19
#define DEFAULT_TEMPERATURE_RECOVER         TEMP_RECTOR_LINEAR //Jimmy added on 2016.03.20
#define DEFAULT_RECTOR_LINEAR_FILENAME       "rector_linear_coef.txt" //Jimmy added on 2016.03.20
#define DEFAULT_RECTOR_REGRESSION_FILENAME   "rector_regression_coef.txt" //Andy added on 2017
#define DEFAULT_RECTOR_REGRESSION1_FILENAME   "rector_regression_coef1.txt" //Andy added on 2017
#define DEFAULT_RECTOR_REF_FILENAME   "rector_regression_ref.txt" //Andy added on 2017
#define DEFAULT_BUFFER_ALLOCATION			NO_BUFFER_ALLOCATION
#define DEFAULT_Throt_Reset_Buffer			NO_Throt_Reset_Buffer
#define DEFAULT_TBDB_Type        	        TBDB_Type_Original
#define DEFAULT_Drop_or_not       	        false
#define DEFAULT_TD_Type        	        	TD_Type_Original //Dora added on 2017.04.17
...
struct NoximGlobalParams {
    ...
    static int                          DQN_x;
    static int                          DQN_y;
    static bool                         DQN_init;
	static bool                         start;
    static bool                         init_throttle;
	...
	static int                          predict_vol; //Jimmy added on 2012.04.15
	static int                          pkt_queue_depth; //Jimmy added on 2012.04.24
	static int                          static_quota;   //Jimmy added on 2012.04.24
	static bool                         selection_info; //Jimmy added on 2012.06.26
	static bool                         message_level;
	static bool                         tla_support; //Jimmy added on 2012.07.15
	static float                        temp_trigger; //Jimmy added on 2012.11.06
	static int                          dfs_level;    //Jimmy added on 2013.03.06
	static int                          sensor_allocate; //Jimmy added on 2015.05.27
    static int                          sensor_allocate_num; //Jimmy added on 2016.03.21
	static char                         sensor_table_filename[128]; //Jimmy added on 2016.03.19
	static int                          temp_recover; //Jimmy added on 2016.03.20
	static char                         rector_linear_coef_filename[128]; //Jimmy added on 2016.03.19
	static char                         rector_regression_coef_filename[128]; //Andy added on 2017
	static char                         rector_regression_coef1_filename[128]; //Andy added on 2017
	static char                         rector_regression_ref_filename[128]; //Andy added on 2017
	static int                          Buffer_Allocation;
	static int  					  	Throt_Reset_Buffer;
	static int                          TBDB_Type;
	static bool                         Drop_or_not;
	static int                          TD_Type;
};
...
struct NoximPacket {
    ...
    double timestamp_ni;    // hfsken 120629
	double timestamp_nw; 	// hfsken 120629
	int    size;
	...
	bool   beltway;
	bool   tampered;		//--added by shubham
    
    // Constructors
    NoximPacket() {
		arr_mid   = false;
		beltway   = false;
		tampered  = false;//--added by shubham
	}
...
struct NoximFlit {
	...
	return (flit.src_id == src_id && flit.dst_id == dst_id
		...
		&& flit.hop_no == hop_no && flit.tampered == tampered);
}};
...
inline ostream & operator <<(ostream & os, const NoximFlit & flit)
{
	...
	os << ", seq: " << flit.sequence_no << ", " << flit.src_id << "-->" << flit.mid_id << "-->" << flit.dst_id << " routing: "<<flit.routing_f<< " tampered: "<<flit.tampered<<"]";
    // if(flit.tampered)
    // 	os<<" FINALLY ";
    if(flit.arr_mid)
		os <<"M-node:0";
...
inline NoximCoord id2Coord(int id)
{
	...
	if(id >= NoximGlobalParams::mesh_dim_x*NoximGlobalParams::mesh_dim_y*NoximGlobalParams::mesh_dim_z)
		id = (NoximGlobalParams::mesh_dim_x*NoximGlobalParams::mesh_dim_y*NoximGlobalParams::mesh_dim_z)-1;
	...
	//cout<<id<<"   "<<coord.x<<"   "<<coord.y<<"   "<<coord.z<<endl;
...
inline string MarkFileName( string name ){
	...
	number_in >> number;
	sprintf( temperal, "%d", NoximGlobalParams::mesh_dim_x);
	name  = name + "_Mesh-" + temperal;
	sprintf( temperal, "%d", NoximGlobalParams::mesh_dim_y);
	name  = name + "-" + temperal;
	sprintf( temperal, "%d", NoximGlobalParams::mesh_dim_z);
	name  = name + "-" + temperal;
	...
	if      ( NoximGlobalParams::beltway ) //Jimmy added on 2017.07.04
		name =name  + "-B";
	
	if(NoximGlobalParams::TBDB_Type == TBDB_Type_Original) //Jimmy added on 2017.06.10
		name =name  + "_TBDB_RTI";
	else if(NoximGlobalParams::TBDB_Type == TBDB_Type_PRE) //Jimmy added on 2017.06.10
		name =name  + "_TBDB_PRE";
	else if(NoximGlobalParams::TBDB_Type == TBDB_Type_GAME) //Jimmy added on 2017.06.10
		name =name  + "_TBDB_GAME";
	
	sprintf( temperal, "%d", NoximGlobalParams::selection_strategy);
	name  = name + "_sel-" + temperal;
	
	sprintf( temperal, "%d", NoximGlobalParams::selection_info); //Jimmy added on 2012.08.19
	name  = name + "_selinfo-" + temperal; //Jimmy added on 2012.08.19
	
	sprintf( temperal, "%d", NoximGlobalParams::TD_Type ); //Jimmy added on 2017.06.10
	name =name  + "-" + temperal; //Jimmy added on 2017.06.10
	
	sprintf( temperal, "%d", NoximGlobalParams::static_quota); //Jimmy added on 2012.08.19
	name  = name + "_tquota-" + temperal; //Jimmy added on 2012.08.19
	
	sprintf( temperal, "%d", NoximGlobalParams::tla_support); //Jimmy added on 2012.08.19
	name  = name + "_tla-" + temperal; //Jimmy added on 2012.08.19
	
	sprintf( temperal, "%d", NoximGlobalParams::predict_vol); //Jimmy added on 2012.08.19
	name  = name + "_PD-" + temperal; //Jimmy added on 2012.08.19
	
	sprintf( temperal, "%d", NoximGlobalParams::throt_type); //Jimmy added on 2012.11.06
	name  = name + "_THROT-" + temperal; //Jimmy added on 2012.11.06
	
	sprintf( temperal, "%.1f", NoximGlobalParams::temp_trigger); //Jimmy added on 2012.11.06
	name = name  + "_Tsh-" + temperal; //Jimmy added on 2012.11.06
	...
	sprintf( temperal, "%d", NoximGlobalParams::traffic_distribution); //Jimmy modified on 2012.09.01
        if      ( NoximGlobalParams::traffic_distribution != TRAFFIC_TABLE_BASED    )
		name = name + "_traffic-" + temperal;
	else name = name + "_traffic-" + temperal + "_" + NoximGlobalParams::traffic_table_filename;
	sprintf( temperal, "%d", number);
	name = name + "_" + temperal +".txt";
	number_in.close();
	return name;
}

inline string shortMarkFileName( string name ){
	char temperal [20];
	int number;
	ifstream number_in;
	number_in.open("number.txt",ios::in);
	...
	sprintf( temperal, "%d", NoximGlobalParams::traffic_distribution); //Jimmy modified on 2012.09.01
        if      ( NoximGlobalParams::traffic_distribution != TRAFFIC_TABLE_BASED    )
		name = name + "-" + temperal;
	else name = name + "-" + temperal + "_" + NoximGlobalParams::traffic_table_filename;
	sprintf( temperal, "%d", number);
	name = name + "_" + temperal +".txt";
	number_in.close();
	return name;
