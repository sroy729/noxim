...
void Thermal_IF::logFile_init()
{
	...
	if( NoximGlobalParams::Log_all_Temp ){
		...
		if(!results_log_temp.is_open())cout<<"Cannot open "<< TEMP_filename.c_str() <<endl;
...
/*** Record the prediction error change withion the simulation (Jimmy added on 2013.03.07) ***/
void Thermal_IF::PredictErrorLog   (){ 
	if(!mkdir("results/PredictError",0777)) cout<<"Making new directory results/PredictError"<<endl;
}
